...
import com.heng.utils.FloatUtils;
...
public class DBManager {
    ...
    public static void initDB(Context context) {
        com.heng.db.DBOpenHelper helper = new com.heng.db.DBOpenHelper(context);  //得到帮助类对象
     ...
     * kind :表示收入或者支出
     */
    public static List<com.heng.db.TypeBean> getTypeList(int kind) {
        List<com.heng.db.TypeBean> list = new ArrayList<>();
        //读取typetb表当中的数据
...
//        循环读取游标内容，存储到对象当中
        while (cursor.moveToNext()) {
            ...
            com.heng.db.TypeBean typeBean = new com.heng.db.TypeBean(id, typename, imageId, sImageId, kind);
            list.add(typeBean);
        }
        return list;
    }
    ...
    public static List<AccountBean> getAccountListOneDayFromAccounttb(int year, int month, int day) {
        List<AccountBean> list = new ArrayList<>();           //集合初始化
        String sql = "select * from accounttb where year=? and month=? and day=? order by id desc";     //查询，按id倒序排列
        Cursor cursor = db.rawQuery(sql, new String[]{year + "", month + "", day + ""});
        //遍历符合要求的每一行数据，年月日已传入，故无需获取
        while (cursor.moveToNext()) {
            int id = cursor.getInt(cursor.getColumnIndex("id"));
            String typename = cursor.getString(cursor.getColumnIndex("typename"));
            ...
            list.add(accountBean);     //将accountBean添加到集合中
        }
        return list;
    }
    ...
    public static float getSumMoneyOneDay(int year, int month, int day, int kind) {
        float total = 0.0f;
        String sql = "select sum(money) from accounttb where year=? and month=? and day=? and kind=?";    //查询总金额
